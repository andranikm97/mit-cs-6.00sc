...
def test_loadSubjects():
    expected = {'6.08': (1, 10), '6.09': (3, 7), '6.19': (8, 19), '6.10': (8, 18), '6.00': (10, 1), '6.01': (5, 4), '6.02': (5, 6), '6.03': (2, 9), '6.04': (1, 2), '6.05': (
        1, 18), '6.06': (5, 19), '6.07': (2, 10), '6.13': (9, 16), '6.18': (10, 4), '6.15': (10, 6), '6.16': (6, 9), '6.12': (6, 3), '6.17': (9, 3), '6.14': (10, 8), '6.11': (6, 8)}
    actual = {}
    failed = False
    try:
    ...
    if test600 != (10, 1):
        ...
        failed = True
    if expected != actual:
        print("ERROR in loadSubjects: Expected " +
...
def test_cmpValue():
    ...
    if type(shouldBeFalse) != bool:
        failed = True
        print("ERROR in cmpValue: return value should be boolean but was actually " +
              str(type(shouldBeFalse)))
...
def test_cmpWork():
    ...
    if type(shouldBeFalse) != bool:
        failed = True
        print("ERROR in cmpWork: return value should be boolean but was actually " +
              str(type(shouldBeFalse)))
    if shouldBeFalse == True:
        failed = True
...
def test_cmpRatio():
    ...
    if type(shouldBeTrue) != bool:
        failed = True
        print("ERROR in cmpRatio: return value should be boolean but was actually " +
              str(type(shouldBeTrue)))
    if shouldBeTrue == False:
        failed = True
...
def test_greedyAdvisor():
    ...
    try:
        ...
        failed = True
        print(
            "ERROR in greedyAdvisor (probably does not handle a new comparator) " + str(e))
    else:
        if expected != actual:
            failed = True
            print("ERROR in greedyAdvisor: For greedyAdvisor(subjects, 17, cmpSum), expected " +
                  str(expected) + " but was actually " + str(actual))
    ...
    try:
        ...
        if expected != actual:
            failed = True
            print("ERROR in greedyAdvisor: For greedyAdvisor(subjects, 8, cmpSum), expected " +
                  str(expected) + " but was actually " + str(actual))

    # Small Catalog handout tests
    print
    smallCatalog = {'6.00': (16, 8), '1.00': (
        7, 7), '15.01': (9, 6), '6.01': (5, 3)}
    ...
    if expected != actual:
        failed = True
        print("ERROR in greedyAdvisor: For greedyAdvisor(smallCatalog, 15, ps9.cmpValue), expected " +
              str(expected) + " but was actually " + str(actual))
    ...
    if expected != actual:
        failed = True
        print("ERROR in greedyAdvisor: For greedyAdvisor(smallCatalog, 15, ps9.cmpWork), expected " +
              str(expected) + " but was actually " + str(actual))
    ...
    if expected != actual:
        failed = True
        print("ERROR in greedyAdvisor: For greedyAdvisor(smallCatalog, 15, ps9.cmpRatio), expected " +
              str(expected) + " but was actually " + str(actual))
    if failed:
        raise Exception
...
def test_bruteForceAdvisor():
    ...
    shortenedSubjects = {'6.00': (10, 1), '6.01': (5, 4), '6.02': (5, 6), '6.04': (
        1, 2), '6.18': (10, 4), '6.15': (10, 6), '6.12': (6, 3), '6.17': (9, 3)}
    ...
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
        print("ERROR in bruteForceAdvisor: For bruteForceAdvisor(shortenedSubjects, 3), expected " +
              str(expected) + " but was actually " + str(actual))
    ...
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
        print("ERROR in bruteForceAdvisor: For bruteForceAdvisor(shortenedSubjects, 4), expected " +
              str(expected) + " but was actually " + str(actual))
    ...
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
        print("ERROR in bruteForceAdvisor: For bruteForceAdvisor(shortenedSubjects, 5), expected " +
              str(expected) + " but was actually " + str(actual))
    shortenedSubjects = {'6.00': (10, 1), '6.01': (5, 4), '6.02': (5, 6), '6.04': (
        1, 2), '6.18': (10, 4), '6.15': (10, 6), '6.12': (6, 3), '6.17': (9, 3)}
    expected = {'6.00': (10, 1), '6.17': (9, 3)}
    ...
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
        print("ERROR in bruteForceAdvisor: For bruteForceAdvisor(shortenedSubjects, 6), expected " +
              str(expected) + " but was actually " + str(actual))
    shortenedSubjects = {'6.00': (10, 1), '6.01': (5, 4), '6.02': (5, 6), '6.04': (
        1, 2), '6.18': (10, 4), '6.15': (10, 6), '6.12': (6, 3), '6.17': (9, 3)}
    ...
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
        print("ERROR in bruteForceAdvisor: For bruteForceAdvisor(shortenedSubjects, 7), expected " +
              str(expected) + " but was actually " + str(actual))

    shortenedSubjects = {'12.02': (5, 5), '12.03': (2, 1), '12.04': (7, 1), '12.07': (
        6, 3), '12.06': (2, 5), '12.08': (2, 4), '12.09': (8, 2), '12.15': (10, 7)}
    print
    print("shortenedSubjects = " + str(shortenedSubjects))
    ...
    shortenedSubjects = {'12.02': (5, 5), '12.03': (2, 1), '12.04': (7, 1), '12.07': (
        6, 3), '12.06': (2, 5), '12.08': (2, 4), '12.09': (8, 2), '12.15': (10, 7)}
    expected = {'12.09': (8, 2), '12.03': (
        2, 1), '12.04': (7, 1), '12.07': (6, 3)}
    print("Trying bruteForceAdvisor(shortenedSubjects, 7)...")
    actual = ps9.bruteForceAdvisor(shortenedSubjects, 7)
    if expected != actual and sumValues(expected) > sumValues(actual):
        failed = True
